...
export async function getStoreProducts(storeSlug: string): Promise<StoreProduct[]> {
  try {
    ...
    const { data } = await supabase
      .from("store_products")
      .select("catalog_item_id, sort_order, item_catalog(id, name, buy_price, placeable)")
    ...
    return (data ?? [])
      .map((row) => {
        ...
        return { catalogItemId: catalog.id, name: catalog.name, price: catalog.buy_price, placeable: catalog.placeable };
      })
