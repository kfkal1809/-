...
export function EventRow({ title, eventId }: { title: string | null; eventId: string | null }) {
  if (!title || !eventId) {
    return (
      <div className="flex items-center gap-2 rounded-full bg-white/70 px-4 py-2.5 text-[13px] text-[var(--color-navy-soft)]">
  ...
  return (
    ...
    >
      <span className="flex items-center gap-2 text-[13px]">
        ...
        <span className="rounded-full bg-[var(--color-gold)] px-2 py-0.5 text-[11px] font-extrabold text-white">
          진행 중 이벤트
        </span>
