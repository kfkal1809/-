...
const FALLBACK_DESCRIPTION = "선실을 꾸며줄 아이템이에요.";

export function FurnitureStoreDetail({
  product,
  balance,
  buying,
  onPreview,
    ...
    <div className="flex items-center gap-3 rounded-[24px] border-2 border-white bg-white p-3 shadow-[0_6px_20px_rgba(36,54,90,0.08)]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2 text-[11px] text-[var(--color-navy-soft)]">
          <span>보유량 {product.ownedQuantity}개</span>
      ...
      <div className="flex shrink-0 flex-col gap-1.5">
        <button
          onClick={onPreview}
          className="rounded-full bg-[var(--color-sky)] px-3 py-1.5 text-[12px] font-extrabold text-[var(--color-navy)]"
        >
          미리보기
        </button>
        <button
        ...
        >
          {buying ? "구매 중..." : canAfford ? "구매하기" : "잔액부족"}
        </button>
      </div>
    </div>
  );
