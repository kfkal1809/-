  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-5">
      <div className="flex items-center justify-between">
        ...
        {data.isOwner && (
          <Link href="/cabin/edit" className="rounded-full bg-[var(--color-navy)] px-4 py-2 text-[12px] font-bold text-white">
            방꾸미기
          </Link>
        )}
      </div>
      ...
      <div className="rounded-[24px] border-2 border-white bg-white p-4 shadow-[0_6px_20px_rgba(36,54,90,0.08)]">
        <div className="flex items-center justify-between">
          <p className="text-[13px] font-extrabold text-[var(--color-navy)]">방명록</p>
          {data.householdId && (
            <Link href={`/cabin/${data.householdId}/guestbook`} className="text-[11px] font-bold text-[var(--color-tab-active)]">
              전체보기 ›
            </Link>
          )}
        </div>
        ...
        {data.guestbook.length === 0 ? (
          <p className="mt-3 flex items-center gap-2 text-[12px] text-[var(--color-navy-soft)]">
