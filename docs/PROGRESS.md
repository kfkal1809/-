...
- **알려진 트레이드오프**: 지금은 캐릭터별로 저장된 `appearance_json`(헤어스타일/색상/의상
  색 커스터마이징)이 시각적으로는 무시된다 — 같은 kind+성별+연령대면 다 같은 그림이 나옴.
  커스터마이즈 화면에서 아이템을 착용해도 DB에는 계속 정상 저장되지만(API/데이터 로직은
  그대로 유지), 화면에 바로 반영되진 않는다. 사용자가 준 헤어/의상 90장 낱개 세트를 이
  기본 체형 위에 정교하게 얹는 작업이 이 트레이드오프를 해소할 다음 단계.
