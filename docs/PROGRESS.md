...
- **`deck.png` 하늘색 배경 문제**: 19개 아이콘 중 `public/images/icons/deck.png`(갑판 광장
  메뉴 아이콘) 하나만 원본 일러스트 자체에 하늘색 사각 배경이 포함되어 있어 투명화가 안 됨
  (다른 18개는 전부 정상). 사용자에게 정확히 어떤 파일인지 전달함 — 투명 배경으로 다시
  그려서 주면 교체 예정.
