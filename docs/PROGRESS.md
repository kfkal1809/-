...
- **남은 백로그**: 시트 2/6/7/10(칸마다 모자+옷+가방+신발 4개 요소라 현재 스크립트의 "칸당
  최대 2개 요소" 가정이 안 맞음 — 신발 분리 로직을 모자 앵커링까지 확장해야 함), 시트
  4/9(캐릭터 몸통 없이 모자/소품만 있는 시트 — 별도 아이템 아이콘 크롭 스크립트 필요)는
  이번에도 처리하지 않음. 선박 일러스트 7종도 여전히 사용자 작화 대기.
- 코드 변경 없음(이미지 자산만 추가) — `tsc`/`vitest run`(83개) 통과 확인.
