...
## 캐릭터 의상 시트 7/10 처리 — 18벌 추가 (칸마다 성별이 다른 시트, 백로그 마지막 남은 것)

이전에 "칸마다 의도된 성별이 달라(교복풍 남아 아이템 + 원피스 여아 아이템 공존) 자동
파이프라인을 시트 전체에 한 번만 못 돌린다"는 이유로 미뤄뒀던 시트 7/10(`design-assets/`에
이미 있던 파일, 다시 요청할 필요 없었음)을 처리했다.

- **처리 방식**: `normalize_dress_overlays.py`를 시트마다 `child_toddler_male`/
  `child_toddler_female` 두 번씩 돌려서(파이프라인이 결정적이라 같은 셀을 두 체형에 합성해도
  문제 없음) 9칸 × 2체형 = 18장씩, 총 36장의 합성 결과를 먼저 전부 만들고, 실제 합성된
  그림(원화 플랫레이가 아니라!)을 보면서 각 칸에 자연스러운 체형을 골랐다 — 시트 7은
  남아 2칸(1, 7)+여아 7칸, 시트 10은 남아 2칸(3, 6)+여아 7칸으로 확정. 우산/선글라스/인형
  등 슬롯이 없는 소품은 기존 `extract_grid` 로직이 이미 자동으로 걸러내(옷+신발만 남김)
  손대지 않아도 됐다. 고르지 않은 쪽 체형의 산출물과 스크립트 중간 작업 파일(`dress/`,
  `dress_shoes/`, 다른 kind 처리 시 tracked 상태가 되돌아가도록 `git checkout --`/
  `git clean`)은 커밋 전 전부 정리.
- **⚠️ 새로 발견한 실제 렌더링 버그(수정함, 이번 18장에서만 발현)**: 시트10 8번 칸(플로럴
  스트랩 원피스)이 합성 후 신발이 가슴 높이에 거대하게 뜨는 버그를 발견 — 원인 추적 결과
  (1) 원화에서 밀짚모자가 원피스 어깨끈과 살짝 겹쳐 있어 셀 분류 단계에서 모자 조각이
  옷 크롭에 얇게 붙어 들어갔고, (2) 무엇보다 이 원피스가 다른 8벌과 달리 아주 가는
  스파게티 스트랩이라 `compose_all`이 어깨폭을 재는 "크롭 상단 30% 구간에서 가장 넓은
  행" 기준이 스트랩만 걸리는 좁은 값을 잡아버려 스케일이 1.52배로 과대 계산됐다(정상
  범위는 대략 0.6~0.9배). 스케일이 커지면 같은 배율을 쓰는 신발도 같이 커지고, 커진
  옷의 세로 길이가 캔버스를 넘어서면서 신발이 몸통 위치로 밀려 올라와 보이는 증상으로
  나타났다. 다른 17장은 전부 정상 범위(0.63~0.85배) — 이 원피스의 얇은 끈 실루엣에서만
  발현되는 국소적 문제라 판단해 공용 스케일 로직은 건드리지 않고(다른 62벌 회귀 위험),
  이 한 장만 (a) 침식(erosion) 기반 형태학적 재구성으로 모자 조각을 분리해낸 크롭을 쓰고
  (b) 수동으로 검증한 스케일(0.95배, 다른 정상 값들과 같은 범위)로 다시 합성해 대체했다.
- **매니페스트/카탈로그**: `lib/domain/itemAppearanceVariants.ts`의
  `OUTFIT_VARIANT_MANIFEST`/`ITEM_APPEARANCE_VARIANTS`에 18개 추가(0010과 완전히 같은
  구조 재사용 — 새 필드/타입 없음). `0019_outfit_variant_pack_2.sql`(신규, 0010과 동일
  패턴)로 `item_catalog` + `store_products` 연결.
- **테스트**: `lib/domain/itemAppearanceVariants.test.ts` — 매니페스트 개수 단언을
  63→81로 갱신, "같은 시트 안에서도 셀마다 다른 체형" 케이스 2개 추가(247개 전체 통과).
- **검증**: 임시 `app/(dev)/outfit-variant-preview`(18종 전부 `resolveAppearancePatch` →
  `CharacterSprite` 실제 렌더링 경로로, 커밋 전 삭제)로 Playwright 확인 — 18개 전부
  `patch` null 없이 정상 해석되고 `pageerror` 0건, 스크린샷으로 (버그 수정한 s10_08
  포함) 18벌 전부 비율 정상 확인. `tsc`/`eslint`/`vitest run`(247개)/`next build` 전부 통과.
  마이그레이션(`0019`)은 이 세션이 라이브 Supabase에 접근할 수 없어 미적용 — 사용자가
  직접 적용 필요(0007~0018과 동일).
- **이걸로 "옷 변형 후속" 백로그는 마무리**: 우산/인형/선글라스처럼 이 게임 캐릭터
  렌더러에 대응 슬롯이 아예 없는 소품은 여전히 범위 밖(손소품과 마찬가지로 목/얼굴
  anchor 같은 새 인프라가 필요하면 그때 별도 작업).

## 목 소품(반다나/보타이) 신규 슬롯 + 우산/인형/선글라스 연결 — 남은 백로그 마지막 정리

"손소품 렌더 슬롯 신설" 때부터 anchor가 없어 미뤄뒀던 목 소품 3종과, "옷 변형 후속"에서
"이 게임 캐릭터 렌더러에 대응 슬롯이 없다"고 범위 밖으로 뒀던 우산/인형/선글라스까지
전부 연결해서 남은 백로그를 마무리했다. 다시 살펴보니 우산/인형/선글라스도 실은
완전히 새 인프라가 필요한 게 아니었다 — 우산/인형은 "손에 드는 물건"이라 기존 손소품
슬롯을 그대로 쓰면 됐고, 선글라스도 이미 있는 고글류(`hat_goggles_*`)와 똑같이 머리에
얹는 소품이라 기존 모자 슬롯을 재사용하면 됐다. 정말 새 인프라가 필요했던 건 목에
두르는 소품(반다나/보타이) 하나뿐이었다.

- **새 슬롯(목 소품)**: `characterFullBody.ts`에 `NECK_SIZE`/`NECK_PLACEMENT`/
  `NECK_ACCESSORY_ANCHOR`(목선 중앙, `OUTFIT_CANVAS_W/2, NECK_Y+5`)/`neckAccessorySrc()`를
  손소품(`HAND_*`)과 완전히 같은 구조로 추가. `CharacterAppearance`에 `neckAssetKey` 필드
  추가(대응하는 구버전 벡터 폴백 필드는 없음 — 처음부터 실사 전용). `CharacterSprite.tsx`에
  손소품 레이어 바로 다음에 목 소품 레이어 추가(같은 계산식 재사용: anchorX/anchorY로
  소품 이미지 안 기준점을 anchor 점에 맞춤).
- **기존 슬롯 재사용**: 우산/인형(`design-assets/캐릭터 의상 (7).png`, 시트7 처리 때 같이
  발견한 소품 — 손에 드는 물건이라 기존 손소품 슬롯 그대로) — `hand_umbrella`/`hand_doll`을
  `HAND_SIZE`/`HAND_PLACEMENT`에 추가. 선글라스(같은 시트 5번 칸)는 기존 `hat_goggles_*`와
  같은 "hat" 슬롯 재사용 — `hat_sunglasses`를 `HAT_SIZE`/`HAT_PLACEMENT`에 추가.
  **⚠️ 처음에 bottomFrac 방향을 반대로 잡은 실수 발견·수정**: "고글류(0.58, 이마 위로
  밀어올린 위치)보다 낮게"라고 생각해서 0.46(더 작은 값)을 넣었는데, 실제로는 bottomFrac이
  클수록 머리 위쪽에서 더 내려온(더 아래) 위치라 반대로 동작 — 라이브 브라우저로 확인하니
  선글라스가 이마 위쪽에 얹혀 보여서 발견, 0.68로 고쳐서 실제로 눈에 걸친 것처럼 보이게
  했다(오프라인 추론만으로는 안 보였을 종류의 실수 — 이 세션 내내 강조된 "라이브 렌더링
  검증 필수" 교훈이 여기서도 그대로 적용됨).
- **크롭**: 목 소품 3종은 기존 "손소품 렌더 슬롯 신설" 때 이미 만들어둔 손소품 시트 크롭
  (`hand_crops/bandana_blue.png` 등, 그때 목 소품이라 손소품 인프라에는 못 넣고 남겨뒀던 것)을
  재사용. 우산/인형/선글라스는 시트7에서 커넥티드 컴포넌트로 새로 정밀 크롭(인접 소품과
  붙어있던 부분은 영역을 좁혀 largest-component만 남기는 방식으로 분리).
- **카탈로그**: `0020_neck_accessory_pack.sql`(신규, 0016/0018과 동일 패턴)로 6개 추가.
  반다나/보타이는 기존 손소품들과 같은 시트에서 나온 성인용 소품이라 haenam_deck/haenyeo/
  haenam_engine으로, 우산/인형/선글라스는 새싹 옷 시트에서 나온 소품이라 subcategory
  `child`로 등록. `itemAppearance.ts`에 6개 매핑 추가(neckAssetKey 3개, handAssetKey 2개,
  hatAssetKey 1개), `itemIcons.ts`에 인벤토리 아이콘 6개 추가(손소품과 같은 방식으로
  크롭본을 `public/images/items/<sku>.png`에 복사).
- **검증**: 임시 `app/(dev)/misc-accessory-preview`(해남/해녀/새싹에 6종 전부 실제
  `CharacterSprite` 렌더링, 커밋 전 삭제)로 Playwright 확인 — 목 소품 3종은 목선에 자연스럽게
  걸리고, 우산은 지팡이처럼 손에 늘어뜨려지고, 인형은 품에 안은 듯, 선글라스는 (수정 후)
  실제로 눈에 걸쳐 쓴 것처럼 보이는 것 확인. `pageerror` 0건.
  `tsc`/`eslint`/`vitest run`(247개)/`next build` 전부 통과. 마이그레이션(`0020`)은 사용자가
  직접 적용하기로 함(이 세션은 라이브 Supabase 접근 불가, 0007~0019와 동일).

### `furniture_rug.png` 재크롭 (바닥 가구 각도 조사 때 발견해둔 잔여 이슈)

- **문제**: 바닥 가구 각도 실험(위 항목) 검증 스크린샷에서 발견하고 그때는 고치지 않고
  넘어갔던 이슈 — `public/images/items/furniture_rug.png` 오른쪽 가장자리에 원본 시트
  크롭 때 같이 딸려온 파란 점선 조각이 남아있었다.
- **원본 시트 특정 불가**: `git log --all -- public/images/items/furniture_rug.png`로 확인한
  결과 이 파일은 이번 세션에서 처리한 시트(해적/요정/마린/코티지/빈티지 등) 어디에도 속하지
  않고, 아주 초기 "Sprint 5" 커밋에서 바이너리만 통째로 추가된 상태 — 대응하는
  `design-assets/*` 원본 시트를 찾지 못했다(당시 파이프라인 이전에 만들어진 파일로 보임).
  원본 재크롭이 불가능해 기존 파일을 직접 정리하는 방식으로 처리.
- **수정**: `scipy.ndimage.label`로 알파 채널 연결요소를 분석해보니 러그 본체(연결요소 1개,
  x 2~172)와 점선 조각들(여러 개의 작은 연결요소, x 172~209)이 서로 완전히 분리돼 있는
  것을 확인 — 러그 본체가 아닌 모든 연결요소를 투명(`RGBA(0,0,0,0)`)으로 지웠다.
  **처음엔 캔버스 자체를 폭 210→175로 잘라내려 했으나, `CabinEditor.tsx`의 선실 배치
  렌더링이 `height:100%; width:auto; object-fit:contain`이라 캔버스 가로세로 비율이 바뀌면
  선실에 놓였을 때 러그가 실제로 17% 정도 작게 렌더링되는 것을 뒤늦게 확인하고 방향을
  바꿨다** — 캔버스 크기(210×163)는 그대로 두고 점선 조각 픽셀만 투명 처리해 배치
  스케일에 영향이 없게 했다. 동일 바이트의 중복 파일(`haenam_engine_acc_wrench.png`/
  `hand_tool_pouch.png` 사례처럼)이 다른 경로에 있는지도 해시로 확인했으나 없었다.
- **범위상 하지 않은 것**: 원본 시트를 못 찾아 고해상도 재크롭은 못 했다 — 기존 저해상도
  파일에서 결함 영역만 지운 것이라 화질 자체는 개선되지 않았다.
...
### 선실 "미니홈피 미니룸" 전면 개편 요청 — 조사 결과 다수 항목이 이미 구현돼 있었음

- **요청**: 선실을 사이월드 미니홈피 미니룸처럼 자유롭게 꾸밀 수 있는 구조로 전면 수정해달라는
  상세 스펙(고정 좌표계, bottom-center 앵커, 드래그 자유배치, 저장 복원, 앞뒤 가림, 보기/편집
  모드 분리, ±15.7도 강제회전 삭제 등)을 받고, 요청대로 먼저 조사 → 원인 설명 → 수정 순서로
  진행.
- **조사 결과 — 이미 구현돼 있던 것들** (스크린샷으로 보여준 화면은 `/cabin`의 **보기 모드**
  (`CabinRoom.tsx`)였는데, 이 화면은 설계상 원래 드래그 핸들이 없는 "완성본 감상용" 화면이고,
  실제 자유배치 편집기는 그 화면의 "방꾸미기" 버튼을 눌러야 나오는 `/cabin/edit`
  (`CabinEditor.tsx`)이었음 — 이미 다음이 전부 동작함을 Playwright로 직접 확인:
  - **배경/가구는 이미 완전히 분리된 레이어**: `room-base.png`(벽+바닥+창문+문만 있는 빈 방)와
    가구 각각의 개별 투명 PNG(`public/images/items/<sku>.png`)가 별개 파일 — 가구가 배경에
    합쳐져 있지 않음. 요청하신 "빈 방 배경 에셋을 찾아 연결" 자체가 불필요.
  - **고정 좌표계 + 모바일/PC 동일 비율**: 가구 위치는 이미 0~1 정규화 좌표를 `aspect-[1473/909]`
    고정비율 컨테이너에 퍼센트로 배치하는 방식이라, 화면 폭이 달라져도 좌표를 다시 계산하지
    않음 — 375px/430px 두 폭에서 렌더링해 방 바운딩박스의 가로세로비가 1.62055로 소수점까지
    완전히 동일한 것을 직접 측정해 확인.
  - **bottom-center 앵커**: `furnitureWrapperStyle`의 `groundAnchorX/Y` 기본값이 이미 (0.5, 1) —
    이미지 중앙이 아니라 바닥 접점 기준으로 통일돼 있었음.
  - **자유 드래그 + 방 밖 이탈 방지**: 편집기에서 가구를 화면 밖(방 바운딩박스 바깥 -200px)으로
    드래그해도 `clampToRoomZone`(바운딩박스 1차 클램프 → 실제 바닥 육각형/벽 평행사변형
    폴리곤 2차 스냅)이 방 안에 붙잡아두는 것을 실제 드래그 테스트로 확인.
  - **저장/취소**: 드래그로 옮긴 뒤 "취소"를 누르면 원래 픽셀 좌표로 정확히 복귀(±2px 이내)하는
    것을 좌표 비교로 확인 — `savedPlaced` 기준선과 `dirty` 플래그로 이미 구현돼 있었음. 저장은
    `space_items` 테이블에 x/y/scale/rotation/flipX/zIndex를 그대로 저장해 재입장 시 복원됨.
  - **좌/우 방향 에셋 교체(회전 아님)**: `furnitureFacingAssets.ts`의 `facing` 시스템이 이미
    "방향별로 실제 다른 그림 파일을 교체"하는 방식으로 구현돼 있음(vintage_shell_bed가
    front/front-left/front-right 3장의 실제 다른 그림을 교체하는 예시로 이미 연결돼 있음) —
    나머지 가구는 원본 시트에 방향별 그림 자체가 없어서(단일 각도로만 그려짐) 확장 못 함(에셋
    한계, 코드 구조는 이미 지원).
  - **앞뒤 가림(z-order)**: `depthOf(y, zIndex)`로 y좌표 기준 깊이를 계산해 CSS `zIndex`에
    반영 — 아래쪽(앞)일수록 위에 그려짐. 캐릭터도 같은 공식을 공유해서 뒤쪽 가구/장식보다
    앞에 표시됨.
  - **보기/편집 모드 분리**: `CabinRoom`(보기, 핸들·테두리 없음)과 `CabinEditor`(편집, 선택 시
    회전/반전/방향/크기/앞뒤/회수 버튼이 뜨는 하단 카드)가 이미 서로 다른 컴포넌트/라우트로
    분리돼 있었음 — 선택 안 한 기본 상태에는 편집 UI가 전혀 안 보이는 것도 스크린샷으로 확인.
  - **공통 좌표계/앵커/드래그 로직**: `furnitureWrapperStyle`/`getPlacementDef`/
    `clampToRoomZone`을 모든 가구가 공유 — 가구별 개별 CSS 보정값을 흩뿌리는 방식이 아님.
- **"±15.7도 강제 회전"에 대한 조사 결과와 의견 차이**: 실제로는 바닥 가구(floor/rug) 회전은
  이미 이전 세션에서 되돌려서 지금 **적용 안 되고 있음**(`wallTiltFor`가 `placementType !==
  "wall"`이면 항상 0 반환). 지금 적용되는 건 **벽 장식(wallDeco)에만** 걸리는 `WALL_TILT_DEG`
  (±13.3도, 15.7도 아님)인데, 이건 "임의로 돌려서 입체감을 흉내내는" 게 아니라 `room-base.png`
  원화 자체에서 벽이 원근으로 기울어져 있는 걸 실측해서 보정하는 값이다 — 이걸 그냥 지우면
  액자/시계 같은 벽 장식이 정면 그림 그대로 기울어진 벽에 얹혀서 다시 붕 떠 보이는, 이전
  세션에서 이미 리포트받고 고쳤던 "벽 각도 미스얼라인" 버그가 재발한다. 그래서 이 값은
  **그대로 유지**했다 — 방향별 에셋이 없는 상태에서 이 보정을 없애는 게 "미니룸답게" 만드는
  방향과 반대라고 판단했다(질문이 아니라 통보성 결정이라, 실제로 벽 장식이 떠 보이면 바로
  알려달라고 코멘트에 남김).
- **실제로 수정한 것**: `components/cabin/CabinRoom.tsx` — 캐릭터가 작고 서로 붙어 보인다는
  지적은 실제로 맞아서, 캐릭터 스프라이트 크기 70→92, 캐릭터 사이 간격 `gap-3`→`gap-6`으로
  키움(보기 모드에서만 — 편집기에는 캐릭터가 없음, 캐릭터는 아직 방 안 자유배치 대상이 아니라
  항상 방 중앙 하단 고정열).
- **범위상 하지 않은 것**: (1) 바닥재/벽지 타일 텍스처의 `backgroundSize`가 고정 px(160px)라
  방 폭이 달라지면 타일 반복 개수가 미세하게 달라지는 것 — 가구 위치와는 무관한 텍스처 표현
  문제이고, 이 값은 "바닥재 모양 불일치" 버그를 어렵게 고친 값이라 손대지 않음. (2) 온보딩
  기본 가구 3종(침대/책상/의자) 좌표 재배치 — 이미 실측 검증된 좌표라 손대지 않았고, 사용자가
  보여준 스크린샷 속 방은 기본 3종이 아니라 이미 구매해 배치한 소품(안락의자/서랍장/아기침대
  등)이 섞여 있어서 배치가 몰려 보인 것 — `/cabin/edit`에서 직접 드래그로 재배치 가능(이미
  동작하는 기능이라 새로 만들 필요 없음). (3) 가구별 좌/우 방향 그림 확장 — 원본 시트에 없는
  방향 그림을 새로 그릴 수 없어 기존 1종(vintage_shell_bed)만 유지.
- **검증**: 임시 `app/(dev)/preview-cabin`(목업 `CabinData`/`PlacedFurniture`로 `CabinRoom`과
  `CabinEditor` 나란히 렌더링, 커밋 전 삭제)을 만들어 Playwright로: (1) 375px/430px 두 폭에서
  방 바운딩박스 가로세로비 동일 확인, (2) 가구 선택 시 편집 컨트롤 카드 등장 확인, (3) 방 밖
  멀리(-200px) 드래그해도 방 안에 클램프되는 것 확인, (4) "취소" 클릭 시 드래그 전 좌표로
  정확히 복귀 확인(다이얼로그 accept 처리), (5) `pageerror` 0건. `tsc`/`eslint`/`vitest
  run`(247개)/`next build` 전부 통과.

### 후속: 가구를 벽 쪽 끝까지 옮겨도 벽에 안 붙는 문제 (진짜 버그였음)

- **문제 재현**: 위 조사에서 "이미 다 된다"고 판단했던 것과 별개로, 사용자가 "가구를 배치하면
  벽에 딱 붙지 않는다"고 재차 지적 — 임시 `app/(dev)/preview-wall-gap`에서 침대를
  `clampToFloorPolygon`이 허용하는 가장 뒤쪽(x=0.22 기준 y≈0.635, 실제 폴리곤 경계 위 좌표
  맞음)까지 Playwright로 드래그해 스크린샷을 찍어보니, 실제로 침대와 벽 사이에 바닥이 한 줄
  더 보이는 눈에 띄는 간격이 있었다 — 클램프 자체(폴리곤 경계에 정확히 스냅되는지)는
  문제없이 동작했는데도 그랬다.
- **원인**: `room-base.png`를 픽셀 단위로 실측해보니(RGB warmth 임계값으로 굽도리널 아래
  진짜 나무 바닥이 시작하는 y를 열 단위로 스캔), 벽-바닥 경계선(굽도리널 아래)이 기존
  `ROOM_CLIP.floor` 폴리곤의 뒤쪽 변보다 화면상 더 위쪽(더 작은 y, 더 뒤쪽)에 있었다. 게다가
  가구 이미지의 anchor는 이미지 전체 bounding box의 bottom-center이지, "가구가 실제로 벽에
  닿는 뒷면"이 아니다 — 침대처럼 헤드보드가 anchor보다 더 위쪽(뒤쪽)까지 뻗어 있는 입체
  가구는, anchor를 정확히 벽-바닥 경계선에 스냅해도 헤드보드 자체는 여전히 그 경계선보다
  앞쪽(진짜 벽보다 화면 아래쪽)에 그려져서 gap이 남는다. 즉 이건 클램프 로직 버그가 아니라
  "폴리곤 경계 = 실측 벽 위치"라는 캘리브레이션 자체가 가구의 입체감(anchor와 뒷면 사이 거리)을
  고려하지 못해서 생긴 오차였다.
- **수정**: `lib/domain/cabinDecor.ts`에 가구 배치 전용 `FURNITURE_FLOOR_CLIP` 폴리곤을
  새로 만들어 `FLOOR_POLYGON`/`ROOM_ZONES.floor`가 이걸 쓰도록 바꿨다(뒤쪽 변 3개 꼭짓점의
  y만 8%p씩 줄여서 뒤로 밀고, 문/캐릭터 쪽 앞쪽 변은 그대로 둠). **`ROOM_CLIP.floor`
  (벽지/바닥재 mix-blend-mode 틴트 마스크용, "바닥재 모양 불일치" 버그를 어렵게 고친 값)는
  건드리지 않았다** — 가구 클램프용 경계와 텍스처 틴트용 경계를 완전히 분리해서, 이번 수정이
  바닥재 색상 입히기 기능에 영향을 주지 않게 했다. 8%p는 침대/책상/책장 등 실제 가구로
  렌더링해보면서 "헤드보드가 벽에 자연스럽게 겹쳐 보이는지, 반대로 가구 앞쪽이 바닥을 벗어나
  이상해 보이진 않는지"를 눈으로 확인하며 정한 값이다(가구마다 실제 "깊이"가 달라 완벽한
  값은 없지만, 대표 가구들로 확인했을 때 이전보다 훨씬 자연스러움).
- **테스트 갱신**: `lib/domain/cabinPlacement.test.ts`의 `clampToPolygon` 테스트 2개가
  하드코딩된 좌표 기댓값(옛 `ROOM_CLIP.floor` 기준)이라 실패 — 새 `FURNITURE_FLOOR_CLIP` 기준
  실제 스냅 결과값으로 갱신(바닥 폴리곤이 실제로 커졌다는 의도된 변화이므로 기존 기댓값이
  아니라 테스트를 새 동작에 맞게 고쳤다). 기본 선실 배치 좌표(침대/책상/의자/냉장고/조명/러그)
  테스트는 폴리곤이 넓어지는 방향의 변경이라 그대로 통과.
- **검증**: 임시 `app/(dev)/preview-wall-gap`(실제 `CabinEditor`에 침대/책상/책장 배치, 커밋
  전 삭제)에서 Playwright로 각 가구를 벽까지 드래그 → 수정 전/후 스크린샷 비교, 벽과 가구
  사이 간격이 눈에 띄게 줄어들고(침대 헤드보드가 굽도리널에 자연스럽게 겹쳐 보임) 세 가구
  모두 부자연스럽게 벽을 뚫고 나가지 않는 것을 확인. `tsc`/`eslint`/`vitest run`(247개,
  갱신된 2개 포함)/`next build` 전부 통과.

### 홈 화면을 목표 시안(`게임 UI 및 메뉴 화면`)에 맞춰 재구성

- **목표 시안 파일 위치**: `design-assets/게임 UI 및 메뉴 화면.png` (941×1672). **이미 영문
  파일명 사본이 존재**했다 — `public/images/misc/home-mockup.png`가 md5 해시까지 완전히
  동일한 파일(`d97d49e5...`)이라, 새로 복사/이름 변경할 필요가 없었다.
- **에셋 대응표** (조사 결과 — 요청하신 배경/장식/아이콘이 전부 이미 낱개 투명 PNG로
  `public/images/home/`, `public/images/icons/`에 존재했고, 대부분 이미 코드에서 쓰이고
  있었다):

  | 화면 요소 | 실제 파일 경로 | 사용 여부(수정 전) |
  | --- | --- | --- |
  | 게임 로고 | `public/images/misc/logo-wordmark.png` | 사용 중 |
  | 등대 | `public/images/home/lighthouse.png` | 사용 중 |
  | 배 | `public/images/home/ship.png` | 사용 중 |
  | 구름 | `home/cloud-1.png` `cloud-2.png` `cloud-3.png` | 1·2 사용 중(3은 미사용) |
  | 갈매기 | `home/seagull-fly-1.png` `seagull-fly-2.png` `seagull-post.png` | 사용 중 |
  | 구명튜브 | `home/lifebuoy.png` | 사용 중 |
  | 갑판 난간 | `home/railing.png` | 사용 중 |
  | 반짝임/장식 점 | `home/deco-sparkle-big/small.png` `deco-dot-blue.png` `deco-heart.png` | 사용 중 |
  | 파도 구분선/하단 배경 | `home/wave-divider.png` `wave-navbar-bg.png` | 사용 중 |
  | 장소 메뉴 아이콘 9종 | `public/images/icons/{deck,clipboard,bag,fishing,chef,flower,gopchang,company,trophy}.png` | 사용 중 |
  | 하단 네비 아이콘 5종 | `public/images/icons/{home,cabin,deck,bag,menu}.png` | 사용 중 |
  | 아이콘 원본 스펙 시트 | `public/images/misc/icon-sheet-source.png` | 참고용(디자이너가 각 아이콘의 용도를 직접 라벨링해둔 시트 — "deck.png는 하단탭·홈메뉴 갑판 둘 다에 쓰는 게 맞다"는 것도 이 시트로 확인) |

  → 결론: "이미지가 반영 안 됨"의 원인은 **에셋 누락이 아니라, 이미 연결된 에셋들의 크기·배치·
  레이아웃 밀도가 시안과 달랐던 것**이었다. 이모지·placeholder·CSS 도형은 원래도 안 쓰고
  있었다(플레이스홀더 문제 아님).
- **반영한 것**:
  1. **레이아웃 전체를 억지로 늘리던 빈 공간 버그(요청 #7) 수정** — `app/(game)/layout.tsx`의
     콘텐츠 컨테이너가 `flex-1`이라, 홈처럼 짧은 화면에서도 뷰포트 높이만큼 늘어나며 장소
     메뉴와 하단 네비 사이에 거대한 빈 하늘색 공간이 생겼다. `flex-1` 제거(콘텐츠는 이제
     자기 내용만큼만 차지) + `BottomNav`를 `sticky`에서 진짜 `fixed bottom-0`(가운데 정렬,
     `max-w-[460px]` 유지)으로 바꿔서, 짧은 화면은 촘촘하게, 긴 화면은 페이지 자체가
     스크롤되며 네비게이션은 항상 화면 하단에 고정되게 했다. `home`/`inventory`/`deck`/
     `menu` 등 여러 화면에서 실제로 스크롤이 잘 되고 마지막 콘텐츠가 고정 네비 뒤에 가려지지
     않는지 확인(콘텐츠 div에 `pb-28`로 네비 높이만큼 여백 확보).
  2. **"나의 항해 정보" 리본 배너화(요청 #4)** — 기존 파란 알약(pill) 양옆에 CSS 삼각형
     "꼬리"를 붙여 리본 팻말처럼 보이게 함(새 이미지 없이 `border` 삼각형 기법).
  3. **캐릭터 확대(요청 #4)** — `VoyageInfoCard`의 해녀/해남 `CharacterSprite` size 115→132.
  4. **장소 메뉴 밀도 개선(요청 #5)** — `grid-cols-3` 3×3(듬성듬성)에서 5개+4개 두 줄로 재배치,
     아이콘도 44→38px로 살짝 줄여 시안처럼 촘촘한 느낌으로. 아이콘은 원래도(수정 전부터)
     `object-fit: contain`, 원형 마스크 없이 투명 PNG 원본 그대로 렌더링하고 있었음
     (`GameIcon` 컴포넌트, 강제 크롭 없음).
  5. 등대/배/구름/갈매기/로고 등 상단 히어로 장식(요청 #3)은 **수정 전부터 이미 낱개 PNG로
     연결돼 렌더링되고 있었다** — 다만 사용자가 실제 배포 화면에서 본 것과 이번 조사에서
     재현한 로컬 렌더가 상당히 달라서(사용자 스크린샷엔 등대/배/구름/갈매기가 거의 안 보임),
     이 차이가 코드 문제가 아니라 **브라우저 캐시나 배포 시점 문제**일 가능성이 있다는 걸
     아래 "확인 못한 것"에 남겨둔다.
- **반영하지 못한 것과 이유**:
  - **아이콘 자체를 시안처럼 바꾸는 것** — 시안의 "갑판 광장" 아이콘은 심플한 파란 닻
    그림처럼 보이지만, 디자이너가 직접 만든 `icon-sheet-source.png`에는 `deck.png`(난간+
    구명튜브 그림)가 "하단탭/홈메뉴 · 갑판" 용도로 명시적으로 라벨링돼 있고 `anchor.png`는
    "메뉴 · 항해일지" 용도로 따로 라벨링돼 있다 — 시안 그림은 단순화된 참고용 스케치이고,
    실제 지정된 에셋은 `deck.png`가 맞다고 판단해 아이콘 자체는 바꾸지 않았다(바꿨다면 오히려
    디자이너 의도와 어긋났을 것).
  - **바닥·문 앞 등 세부 여백 완전 동일화** — 시안은 정적 목업(실제 반응형 그리드가 아님)이라
    카드 비율/모서리 곡률 등을 픽셀 단위로 100% 맞추지는 않았고, 기존 손그림 테두리·카드
    스타일(둥근 모서리, 흰/크림 패널, 파란 포인트)은 이미 시안과 같은 톤이라 유지했다.
  - **선용금 정보판을 시안처럼 완전한 원목 액자 그림으로 교체** — 현재도 점선 테두리+크림
    배경으로 비슷한 느낌을 내고 있고, 전용 액자 프레임 이미지 에셋은 찾지 못해 새로 그리지
    않았다(요청사항의 "임의 생성 금지" 원칙에 따름).
- **검증**: `app/(dev)` 임시 라우트 없이 실제 `/home` 페이지를 Playwright로 직접 렌더링
  (데모 데이터 경로) — 390×844, 430×932 두 해상도에서 스크린샷, 스크롤 시 마지막 장소 메뉴
  줄이 고정 네비 위로 완전히 올라오는 것 확인, `pageerror` 0건. `/inventory`, `/deck`,
  `/menu`도 같은 레이아웃 변경 후 정상 렌더링 확인(다른 화면 안 망가짐). `tsc`/`eslint`/
  `vitest run`(247개)/`next build` 전부 통과.
- **확인 못한 것 (사용자 재확인 필요)**: 사용자가 보내준 실제 배포 화면 스크린샷은 이번에
  로컬에서 재현한 화면보다 장식이 훨씬 적어 보였다(등대/배/구름/갈매기가 거의 안 보임,
...
### 파트너 연결 코드 기능 신설 — "뒤바뀜 버그"의 근본 원인이었던 빈 자리를 실제로 메움

- **배경**: 위 캐릭터 감사에서 "실제 상대가 나중에 가입해도 플레이스홀더 캐릭터를 넘겨받을
  방법이 없다"고 남겨뒀던 부분을 마저 구현. 초대코드(가입 게이트) 제거로 끊어졌던 "두 사람을
  같은 household로 합치는 경로"를, 가입 자체는 막지 않으면서 선택적으로 연결만 하는 방식으로
  다시 만들었다.
- **DB**: `0021_household_join_code.sql`(신규) — `households.join_code`(6자리 대문자+숫자,
  유니크) 추가. 기존 household는 마이그레이션 시점에 각자의 `id`에서 뽑아 일괄 채우고, 이후
  새로 만드는 household는 컬럼 `default` 표현식으로 자동 발급(앱 코드가 따로 생성 안 해도 됨).
- **가입 흐름**: `auth/callback`의 리다이렉트를 `/onboarding/me` → **`/onboarding/join`**(신규,
  온보딩 맨 앞)으로 변경. 이 화면에서 상대에게 받은 코드를 입력하면(`POST /api/onboarding/
  join-household`, 신규) 아직 자기 household가 없는 사람만 그 household의 구성원으로
  들어간다(household를 이미 가진 사람이 코드로 갈아타는 건 지원 안 함 — 지갑/가구까지
  옮기는 별도 마이그레이션 작업이라 범위 밖). 코드가 없으면 "코드 없이 새로 시작할게요"로
  기존과 동일하게 진행.
- **캐릭터 넘겨받기**: `/api/onboarding/character`(내 캐릭터 만들기)에 로직 추가 — 같은
  household 안에 내가 고른 kind와 같은 `managed_only=true` 플레이스홀더가 있으면 새로
  만들지 않고 그 캐릭터를 그대로 넘겨받는다(닉네임/외형 갱신 + `managed_only=false` + 나를
  매니저로 추가). 이미 `voyages` 같은 연결 데이터가 있으면 그대로 유지됨(재생성 안 함).
- **중복 방지**: `/onboarding/partner`("상대 캐릭터도 만들어요") 단계는 코드로 들어와서
  household에 이미 실제 캐릭터(managed_only=false) 2명이 다 있으면 자동으로
  `/onboarding/children`으로 건너뛴다(`GET /api/onboarding/partner-needed` 신규) — 안 그러면
  이미 둘 다 있는데 또 플레이스홀더를 만드는 사고가 난다. 안내 문구도 이제는 사실이 아닌
  "초대코드로 연결"이라는 말 대신 "완료 화면에서 연결 코드를 받아 가입하면 넘겨받아요"로 정정.
- **코드 확인 지점 2곳**: (1) 온보딩 완료 화면(`/onboarding/complete`)에 내 household의
  연결 코드를 바로 보여줌(`GET /api/household/join-code` 신규), (2) 나중에 다시 보고 싶을 때를
  위해 메뉴에 "파트너 연결 코드"(`/menu/join-code`, 신규) 항목 추가.
- **덤으로 발견한 같은 유형의 버그 1개 더**: `app/(game)/menu/page.tsx`의 "나의 승선확인증"
  링크가 `character_managers`를 정렬 없이 1건만 가져오는 **똑같은 패턴**이라, 이것도 파트너
  플레이스홀더로 잘못 열릴 수 있었다 — `getDeckSelf()`와 같은 방식(`managed_only=false`
  필터)으로 같이 고쳤다. 프로젝트 전체에서 `character_managers`를 쓰는 나머지 8곳도 다시
  확인했는데, 나머지는 전부 (a) 특정 characterId에 대한 권한 확인이거나 (b) "내가 관리하는
  캐릭터 전체 목록"이 목적이라 이 버그와 무관했다(둘 다 모호함이 생기지 않는 패턴).
- **범위상 하지 않은 것**: 이미 자기 household를 가진 사람이 나중에 다른 household로
  옮겨가는 것(코드 입력으로 갈아타기)은 지원하지 않는다 — 지갑 잔액/가구/인벤토리를 어느
  쪽에 남길지 정책 결정이 필요한 별도 기능이라 요청 시 따로 설계. 온보딩 단계 표시
  ("STEP 2/4" 등)가 `/onboarding/join`이 새로 앞에 추가되면서 실제 단계 수와 안 맞게 됐는데
  (5단계인데 4로 표시), 기능에는 영향 없는 표시 문제라 이번엔 손대지 않음.
- **검증**: `/onboarding/join`, `/menu/join-code`를 Playwright로 렌더링해 정상 표시 확인
  (로그인 세션이 없는 이 환경에서는 각각 "코드 없이 시작"/"코드를 불러오지 못했어요" 에러
  상태까지 정상적으로 처리되는 것 확인 — 크래시 없음). 실제 두 계정으로 코드 입력 → 같은
  household 진입 → 플레이스홀더 캐릭터 넘겨받기까지의 전체 흐름은 라이브 Supabase가 없어
  이 세션에서 직접 실행해보지 못했다 — **마이그레이션 적용 후 실제 두 계정으로 꼭 한번
...
### 새싹 헤어스타일이 선택과 안 맞는 문제 — "bun"이 그림 없이 선택만 가능했음

- **원인**: 훨씬 이전 세션("헤어스타일 커스터마이징" 작업, PROGRESS.md에 이미 정직하게
  기록돼 있었음)에서 새싹 헤어 오버레이 10종 중 올림머리(번헤어) 그림이 없어서
  `HAIR_STYLE_INDEX.child`에 `bun`을 매핑하지 않고 "민머리+고정 head 그림 폴백"으로 처리해둔
  채였다. 그런데 `ChildCharacterForm`/`CHILD_HAIR_STYLES`에는 `bun`이 여전히 선택 가능한
  버튼으로 남아있어서, 골라도 실제로는 다른(고정된) 헤어스타일이 나오는 상태였다 — "헤어랑
  머리랑 안 맞는다"는 리포트와 정확히 일치. 실제로 새싹 헤어 오버레이 10장을 민머리 베이스에
  다 합성해서 직접 비교해봤는데, 올림머리 형태의 그림은 정말 하나도 없는 것도 재확인했다.
- **수정 1**: `components/onboarding/swatches.ts`의 `CHILD_HAIR_STYLES`에서 `bun` 제거(단발/
  트윈테일/포니테일 3종만 남김 — 이 3개는 전부 실제 그림이 있고 정상 작동 확인됨). 그림 없는
  옵션을 계속 보여주는 것보다, 실제로 있는 것만 보여주는 게 맞다고 판단(이 세션 내내
  적용해온 "안 되는 걸 약속하지 않는다" 원칙과 동일).
- **수정 2**: 새싹도 "피부톤/헤어/의상컬러 다시 설정"을 쓸 수 있게 확장. 지난번 이 기능을
  처음 만들 때는 "새싹은 별도 실루엣 체계라 범위 제외"라고 명시적으로 뺐었는데, 다시 보니
  새싹도 해녀/해남과 똑같은 `outfitAssetKey`/헤어 오버레이 시스템을 쓰고 있어서 못 뺄 이유가
  없었다 — `BaseAppearanceEditor`에 `kind === "child"` 분기(스와치는 `CHILD_HAIR_STYLES`/
  `CHILD_OUTFIT_SWATCHES`/`CHILD_OUTFIT_ASSET_BY_SWATCH`, 미리보기에 `childGender`/
  `childStage` 전달) 추가, `CustomizeScreen.tsx`의 `kind !== "child"` 제외 조건 제거,
  `/api/character/base-appearance`의 `unsupported_kind` 차단도 제거. 이제 방금 만든 새싹의
  머리가 마음에 안 들면 새싹 캐릭터 꾸미기 화면에서 바로 다시 고를 수 있다.
- **검증**: 임시 `app/(dev)/preview-child-editor`(새싹 `CustomizeScreen` 렌더링, 커밋 전
  삭제)를 Playwright로 열어 "다시 설정" 버튼 클릭 → 헤어 스타일 버튼이 정확히 3개(단발/
  트윈테일/포니테일, `bun` 없음)만 뜨는 것 확인, 3개를 각각 클릭해 미리보기 캐릭터의 헤어
...
로고 아래 별도로 떠 있던 `lighthouse.png`/`ship.png` 가로 행(사용자가 스크린샷으로 지적)을
`HomeHeader.tsx`에서 삭제 — 구름/갈매기/로고/선용금 카드/wave-divider만 남김.

## 아이콘 클릭 반응속도 개선

`app/(game)/loading.tsx` 추가. App Router는 loading.tsx가 없으면 다음 화면의 서버 데이터
조회(Supabase 쿼리)가 끝날 때까지 클릭에 대한 시각적 반응 없이 이전 화면을 그대로 두다가
한 번에 전환한다 — "아이콘을 눌러도 한참 있다 넘어간다"는 느낌의 원인. 이 파일을 추가해
클릭 즉시 AppFrame/BottomNav는 유지한 채 콘텐츠 영역에 스피너를 보여주도록 했다(실제
서버 조회 시간 자체를 줄이는 건 아니지만 클릭에 대한 반응 자체는 즉시 눈에 보인다).

## 캐릭터 착용 시스템 구조 조사 + 해녀/해남 키 불일치 구조적 버그 수정 (1단계)

사용자가 헤어/의상/신발/액세서리가 화면마다 다르게 보이고 옷을 바꾸면 캐릭터 키가 달라지는
문제를 구조적으로 고쳐달라고 요청 — 14단계짜리 상세 스펙(마스터 캔버스, 앵커, 정규화,
전수검사)을 줬다. 이번 세션에서 실제로 한 것과 못한 것을 정직하게 구분해서 기록한다.

**1. 실제 렌더링 구조 조사** — `grep -rl "CharacterSprite"`로 전체 저장소를 뒤져 실제 사용되는
캐릭터 렌더러가 `components/character/CharacterSprite.tsx` 단 하나뿐임을 재확인(14개 호출부
전부 이 컴포넌트를 씀 — 홈/선실/꾸미기/갑판/낚시터/온보딩/피팅룸/승선확인증 등). 이 컴포넌트
안에 실제로는 3가지 렌더링 경로가 있다:
- `fullPortraitKey` 있음 → `dress_full/*.png`(원피스류, 기본 체형 원본 위에 완성 합성된 한 장)
- `outfitAssetKey` 있음 → `outfit_full/*.png`(목 아래, 420x512 정규화 캔버스) + 별도 레이어(머리/
  헤어/모자/손소품/목소품)를 앵커 좌표로 조합 — 상의/하의/유니폼류가 전부 이 경로.
- 그 외(`kind`만) → 기본 사진(`base/*.png`) + 색상 마스크 틴트만.
(폴백용 구버전 SVG 벡터 렌더러도 파일 안에 남아있지만 `kind`가 항상 넘어와서 실제로는 절대
안 쓰임 — 지난 세션에 이미 확인된 내용 재확인.)

앵커/배치 메타데이터는 이미 `lib/domain/characterFullBody.ts`에 있다 — `HEAD_SIZE`,
`HAIR_ASSET_PLACEMENT`(헤어별 폭/좌우/상하 비율), `HAT_PLACEMENT`, `HAND_PLACEMENT`,
`NECK_PLACEMENT` 전부 "렌더된 머리 폭/높이 대비 비율" 형태의 아이템별 메타데이터 방식이라,
사용자가 요청한 "CSS 땜질이 아니라 데이터 기반 앵커" 구조 자체는 이미 있었다. 문제는 구조가
아니라 그 안의 **실측되지 않은 값**들이었다(코드 주석에 이미 "라이브 브라우저로 픽셀 단위까지
검증 못해 어림값"이라고 정직하게 적혀 있었음).

DB: 착용 정보는 `characters.appearance_json`(JSONB, `CharacterAppearance` 타입)에 저장 —
`outfitAssetKey`/`hatAssetKey`/`handAssetKey`/`neckAssetKey`/`hairStyle`/`hairColor` 등을
그대로 담는다. 별도 마이그레이션 없이 기존 필드 그대로 사용(변경 없음).

**2~3. 에셋 실측 + 마스터 앵커** — PIL로 실측:
- `base/haenyeo.png`(357×772)/`base/haenam.png`(395×700, 전신 기본 사진)와
  `base/head/*.png`가 정확히 같은 그림(2D 상관관계 탐색으로 haenyeo는 offset (x=6,y=0),
  haenam도 (x=6,y=0)에서 diff=0.00, 즉 완전 일치)임을 확인 — head 크롭은 전신 사진의
  y=0(맨 위)부터 시작한다.
- `outfit_full/*.png` 10종 표본(해녀/해남 갑판/기관 옷 다양하게)의 알파 bbox를 쟀더니
  **전부 top=140(=NECK_Y 상수와 정확히 일치), bottom=490으로 완전히 동일** — 발 위치
  정규화 자체는 이미 잘 돼 있었다(개별 옷마다 발 높이가 다르다는 가설은 틀렸음, 실측으로
  기각).
- 대신 `CharacterSprite`가 머리 위 여유 공간을 계산하는 `HEAD_MARGIN_TOP`이 **모든 kind에
  공통인 고정값(90)**이었는데, 실제로는 해녀 머리(346×425, 세로로 긴 올림머리 크롭)와 해남
  머리(384×354, 가로로 넓은 크롭)의 종횡비가 완전히 달라서 HEAD_WIDTH(190)로 폭을 맞추면
  해녀 머리는 233px 높이, 해남 머리는 175px 높이만 나온다 — 90이라는 값은 해녀 머리 높이에
  맞춰진 값(정확히는 91.4가 필요)이라 해남/새싹 6종에는 전혀 안 맞았다.
- 임시 미리보기 페이지(`app/(dev)/preview-character-scale`, 커밋 전 삭제)를 만들어
  Playwright로 실제 렌더된 `<img>` DOM 크기를 측정 — 같은 `size=220`에서 해녀 outfitAssetKey
  캐릭터는 박스의 96%를 채우는데 해남은 88%만 채움(박스 위에 20px 빈 공간). "옷을 바꾸면
  캐릭터 키가 달라져 보인다"는 사용자 지적이 실측으로 확인됨.
- **수정**: `HEAD_MARGIN_TOP`(고정값)을 `HEAD_MARGIN_TOP_BY_KIND`(kind별 실측값,
  `lib/domain/characterFullBody.ts`)로 교체 — 해녀 91.4/해남 33.2/새싹 6종 각각 실제 머리
  종횡비로부터 역산한 값. `CharacterSprite.tsx`의 `outfitAssetKey` 렌더링 경로에서
  `headMarginTopFor(headKey)`로 조회해서 씀. 재측정 결과 해남이 88%→96.7%로 개선되어
  해녀(95.8%)와 거의 같아짐(스크린샷으로 확인, 실제 `/home` 화면에서도 해녀 두부/해남
  북극곰이 이제 같은 눈높이에 나란히 섬).

**헤어 정렬(요청 5번, 특히 지목된 2건)** — 지난 세션에 "해녀 올림머리가 얼굴에서 위/왼쪽으로
어긋나 보인다", "해남 옆머리가 턱 밑까지 내려와 수염처럼 보인다"고 기록돼 있던 문제를 재현
시도. 임시 헤어 비교 그리드 페이지(해녀 5종/해남 4종 전체, 커밋 전 삭제)로 확인한 결과,
**두 문제 모두 재현되지 않았다** — 모든 헤어가 얼굴 중심에 정렬돼 있고, 옆머리가 턱까지
내려오는 것도 없었다. (참고: 처음 이 페이지를 만들 때 비교용 중심선을 셀 너비의 50%에
그렸는데, `CharacterSprite`의 실제 렌더 폭이 셀 너비보다 좁아서 중심선 자체가 틀어져 있었고,
그래서 처음엔 "얼굴이 왼쪽으로 어긋나 보인다"는 잘못된 결론을 낼 뻔했다 — `justify-content:
center`로 고친 뒤 재측정해서 바로잡았다. 이 경험 자체가 "PNG 크기가 같다고 규격이 같다고
판단하지 마라"는 사용자 지적과 같은 종류의 함정이라 기록해둔다.) 즉 이번 실측으로는 그
2건이 재현되지 않았다 — 만약 실제 화면에서 여전히 보인다면 이 세션의 테스트 방법으로는
못 잡아낸 것이니 스크린샷과 함께 다시 알려주면 좋겠다.

**완료 못 한 것(정직하게 기록)**: 이번 세션에서는 시간 제약상 다음을 하지 못했다 —
(a) 의상 좌우 폭/신발/모자/손소품/목소품 각 에셋의 개별 픽셀 실측 재검증(현재 값들은
여전히 코드 주석에 명시된 대로 "어림값" 상태), (b) 정규화된 별도 자산 폴더 생성(이번 수정은
좌표 상수(`HEAD_MARGIN_TOP_BY_KIND`)만 고쳤고 PNG 자체를 재배치하지는 않았다 — 애초에 이번에
찾은 버그가 이미지 자체가 아니라 렌더러의 여백 계산값 문제였기 때문), (c) 모든 헤어/의상/
신발/액세서리 조합을 한 화면에서 비교하는 상시 개발용 QA 화면(임시로 만들었다가 삭제함,
재사용 가능한 형태로 남기지 않음), (d) 원피스(`fullPortraitKey`) 경로와 일반 의상
(`outfitAssetKey`) 경로 사이의 잔여 스케일 차이(실측: 해녀 기준 95.8% vs 94.4%, 약 1.4%p —
처음 발견한 88% vs 96%(8%p) 격차에 비하면 훨씬 작지만 0은 아님, 두 경로가 서로 다른 생성
파이프라인을 쓰기 때문에 완전히 없애려면 원피스 합성 스크립트 쪽도 손봐야 함), (e) 갑판/
낚시터/커플 화면/프로필 등 나머지 실제 화면 전체에서의 개별 재검증(모두 같은
`CharacterSprite`를 쓰므로 구조적으로는 이 수정이 똑같이 적용되지만, 화면마다 스크린샷을
찍어 눈으로 확인하지는 못했다). 이 항목들은 후속 작업으로 남겨둔다.

## 캐릭터 착용 시스템 — 모자/손소품/목소품 전수검사 (2단계)

1단계(해녀/해남 키 불일치 수정)에 이어, 남겨뒀던 항목 중 모자 24종·손소품 14종·목소품 3종을
실제로 전수검사했다.

- **사전 점검**: 코드의 `HAT_SIZE`/`HAND_SIZE`/`NECK_SIZE`에 적힌 가로·세로 값이 실제 PNG
  파일 크기와 정확히 일치하는지 스크립트로 전수 대조 — 41개 전부 일치, 파일 누락도 없음.
  각 파일의 알파 bbox도 재서 내부 투명 여백을 확인 — 모자 24종은 전부 4방향 5~6px로 균일한
  여백(같은 크롭 파이프라인에서 나왔다는 뜻, 개별 여백 불균형에 의한 어긋남 아님), 손소품/
  목소품은 대부분 여백 0%(꽉 찬 크롭). `hand_fishing_rod`만 오른쪽에 14%의 비대칭 여백이
  있었지만 anchor가 반대쪽(왼쪽, anchorX 0.02)에 있어 실제 배치에는 영향 없음.
- **개발용 검수 화면 신설(영구 보존)**: `app/(dev)/qa-wear/hats`, `app/(dev)/qa-wear/hands`
  — 해녀/해남 각각에 모자 24종 전체, 손소품 14종 전체, 목소품 3종을 하나씩 입혀 한 화면에서
  비교할 수 있게 만들었다. 다른 임시 미리보기 페이지와 달리 이번엔 **삭제하지 않고 커밋에
  포함**했다(사용자가 명시적으로 "전수검사 화면을 만들어달라"고 요청) — 단
  `components/ships/ShipEventOverlay.tsx`의 `SHOW_DEBUG_TRIGGER`와 같은 패턴으로
  `process.env.NODE_ENV === "production"`이면 `null`을 반환해 실제 서비스에는 노출되지 않는다.
- **검사 결과**: 48개 모자 조합(24종×2 kind), 28개 손소품 조합(14종×2 kind), 3개 목소품
  조합을 Playwright로 개별 셀 캡처해 확대 확인 — 목이 잘리거나 크게 어긋난 항목, 얼굴을
  가리는 항목, 캐릭터 밖으로 튀어나온 항목을 찾지 못했다. 작은 헤어핀류(닻/조개/불가사리/
  데이지/보우 클립 8종)도 머리 옆쪽 적절한 위치에 좌우 번갈아 붙어 있는 것을 확인했다.
  → **이번 검사에서는 추가로 고쳐야 할 개별 아이템을 찾지 못했다.** 1단계에서 고친
  `HEAD_MARGIN_TOP_BY_KIND` 버그가 실제로는 가장 큰 원인이었고, 나머지 배치값(어림값으로
  기록돼 있던 것들)은 시각적으로는 이미 합리적인 수준이었던 것으로 보인다.
- **정직하게 남겨두는 것**: 이번 검사는 "명백히 깨진 항목이 있는가"를 스크린샷으로 눈으로
  확인한 것이지, 사용자가 요청한 것처럼 각 항목의 픽셀 좌표를 마스터 앵커 기준으로 재실측해
  메타데이터 값 자체를 새로 계산한 것은 아니다 — 즉 "찾아서 고쳤다"가 아니라 "찾아봤는데 이번
  기준으로는 더 고칠 게 안 보였다"는 결과. 또한 원피스(`fullPortraitKey`) 경로의 잔여 스케일
  차이(1.4%p)와 갑판/낚시터 등 나머지 실제 화면에서의 개별 스크린샷 검증은 이번에도 하지
  못했다.

## 캐릭터 착용 시스템 — 실제 화면 추가 검증 (3단계)

"진행해"라는 요청에 이어 나머지 실제 화면에서의 검증을 더 진행했다.

- **`/cabin`(선실) 확인 완료**: `lib/game/cabinData.ts`에도 `/home`과 같은 비로그인 시
  데모 폴백(`DEMO`)이 있어서 실제 화면을 그대로 캡처할 수 있었다. 두부(해녀)·북극곰(해남)이
  같은 눈높이로 나란히 서 있는 것을 확인 — 1단계 수정이 `/home` 외에 `/cabin`에서도 똑같이
  적용됨을 실제 화면으로 재확인.
- **`/deck`(갑판광장)·`/fishing`(낚시터)은 로그인 없이 캐릭터가 아예 렌더링되지 않음** —
  두 화면 다 데모 폴백이 없어서(로그인 안 한 상태에서는 "로그인 후 대화/낚시할 수 있어요"
  안내만 뜨고 캐릭터 자체가 안 나옴) 이 세션(실제 로그인 세션이나 시드 데이터 없음)에서는
  실제 화면으로 재현할 방법이 없었다. 코드상으로는 `DeckScreen`/`FishingScreen` 둘 다
  `CharacterSprite`를 화면 전용 크기 보정 없이 그대로 쓰고 있어서(추가 `transform`/`scale`
  CSS 없음, 확인함) 구조적으로는 1단계 수정이 똑같이 적용되지만, 실제 캡처로 두 눈으로
  확인하지는 못했다 — 로그인 세션이 있는 환경에서 직접 확인해달라고 요청드린다.
- **`/character/[id]/customize`(캐릭터 꾸미기)도 characterId가 있는 실제 캐릭터가 있어야
  해서 로그인 없이는 렌더링 안 됨.** 코드 확인(`components/character/CustomizeScreen.tsx`
  80번째 줄 부근)으로는 `<div className="flex justify-center">` 안에 `CharacterSprite`를
  그대로 넣을 뿐 별도 크기 계산이 없어서 구조적으로 문제없어야 하지만, 이것도 실제 캡처로는
  확인 못 했다.
- **원피스 잔여 스케일 차이(1.4%p)**: 두 렌더링 경로(`outfitAssetKey`/`fullPortraitKey`)의
  수식을 다시 따라가 보니 둘 다 최종적으로 `innerHeight = size * kindScale`로 동일한 공식을
  쓰고 있어서, 이론상 두 경로의 "박스 안에서 차지하는 높이"는 같아야 한다 — 남은 1.4%p 차이는
  각 경로가 참조하는 원본 그림 자체의 미세한 내부 여백 차이(`outfit_full`은 헤어/모자까지
  포함한 레이어 합성이라 상단 buffer를 8px로 명시 조정했지만, `dress_full`은 기본 체형 사진
...
## 상의·하의·신발 독립 레이어 조사 (실제 착용 시스템 확장 검토, 구현 아님)

"CharacterSprite를 상의·하의·원피스·신발·헤어를 독립적으로 조합 가능한 레이어 구조로
확장해달라"는 요청에 따라, 저장소의 모든 캐릭터 의상 원본(디자인 시트)과 정규화된 PNG를
전수 확인했다. 결론: **하의/신발이 독립된 투명 에셋으로 존재하지 않는다.** 이유와 근거,
그리고 "빈 탭 숨기기 + 기존 전신 의상 재분류"만 이번에 실제로 구현한 내용을 아래에 남긴다.

### 조사 방법과 근거

- `public/images/character/` 전체 트리(`base/`, `haenyeo/`, `haenam/`, `child*/`,
  `outfit_full/`, `dress_full/`)를 열어서 확인.
- 원본 디자인 시트도 직접 열어서 확인: `design-assets/해녀 의상.png`(해녀 22벌 전체),
  `design-assets/기관사 항해사 해남이 의상 (1~4).png`(해남 20벌 전체), `새싹 헤어 및
  의상.png` 계열, `캐릭터 의상 (1~10).png`(원피스류 소스).
- 코드 grep: `ITEM_APPEARANCE_PATCH`(itemAppearance.ts), `CharacterSprite.tsx`의 실제 레이어
  합성 순서, `scripts/asset-tools/normalize_outfits.py` /
  `normalize_dress_overlays.py` 두 파이프라인 스크립트, `item_catalog` 테이블 CHECK 제약.

### 실제 에셋 상태

1. **"outfit" 카테고리(해녀 20종 + 해남 항해사 10종 + 해남 기관사 10종 + 새싹 10종,
   `outfit_full/*.png`)**: 원본 디자인 시트 단계부터 이미 "목 아래 전신 인형" 한 장으로
   그려져 있다 — 예: 해남 시트 라벨 자체가 "③반팔 티셔츠 & 카고팬츠", "⑩레인코트 & 장화"처럼
   상의+하의(+신발)를 한 벌로 명명해뒀고, 그림도 팔·몸통·다리·신발이 전부 하나의 실루엣으로
   붙어 있다(어깨~발끝 분리선 없음). `outfit_full/haenyeo_outfit_02.png`,
   `outfit_full/child_outfit_02.png`를 직접 열어 확인 — 흰 신발이 바지 밑단에 그대로
   그려져 있다. **상의·하의·신발이 물리적으로 한 장의 그림이라, 자르지 않고는 분리 자체가
   불가능하다.**
2. **"dress" 카테고리(해녀 9종 + 새싹 다수, `dress_full/*.png`)**: 소스는 팔다리 없이
   원피스 실루엣 + 그 아래 떠 있는 신발을 따로 그린 3x3 그리드 시트(`캐릭터 의상 (N).png`)다.
   `normalize_dress_overlays.py`가 이 시트에서 원피스와 신발을 각각 크롭해
   `public/images/character/<kind>/dress/dress_XX.png`,
   `<kind>/dress_shoes/shoes_XX.png`로 뽑은 뒤 기본 체형 위에 합성해
   `dress_full/<kind>_dress_XX.png`(완성 전신 그림)를 만든다. **`dress/`, `dress_shoes/`
   폴더는 앱 코드 어디서도 참조하지 않는 스크립트 중간 산출물**이고(`fullPortraitKey`로
   참조되는 건 `dress_full/*.png`뿐, grep으로 재확인), 같은 kind로 다른 시트를 다시 돌리면
   이전 결과가 조용히 덮어써지도록 스크립트가 짜여 있어(docs 상단 기록 참고) 안정적인
   재사용 대상이 아니다. 그리고 원피스 자체는 상의+치마가 이미 하나로 붙은 실루엣이라, 있다
   해도 "하의"가 아니라 "원피스"용 부분 소재일 뿐이다.
   - 예외적으로 `public/images/character/haenyeo/dress_shoes/shoes_01~08.png` 8장은 실제로
     신발만 홀로 잘려 있는(원피스 실루엣과 분리된) 파일로 지금도 디스크에 남아 있다. 다만
     ⓐ해녀+원피스 조합에서만 나온 것이라 해남·새싹·해녀 오버롤류에는 대응 파일이 없고,
     ⓑ공용 마스터 캔버스(`OUTFIT_CANVAS_W/H=420×512`, `NECK_Y=140`) 기준으로 정규화된 게
     아니라 그때그때 합성 대상 크기에 맞춰진 스크립트 내부 크롭이며, ⓒ파일명-원피스 대응이
     코드로 보장되지 않는다(문서화된 규칙 없이 스크립트 실행 순서에 의존) — "독립 신발
     레이어 시스템"의 시작점으로 쓰기엔 부족해서 그대로 두고 보고만 한다.
3. **기본 베이스(`base/haenyeo.png`, `base/haenam.png`, `base/child_*.png`)**: 이미 맨발이고
   상의/하의는 러닝셔츠+반바지 형태의 "속옷"만 그려져 있다(신발 없음). 즉 신발 레이어를
   새로 만든다면 베이스 쪽은 이미 맨발이라 추가로 벗겨낼 신발이 없다 — 문제는 신발 원화가
   없다는 것이지 베이스가 막고 있는 게 아니다. 다만 상의/하의 쪽은 "속옷"이 이미 그려져 있어
   완전한 알몸 실루엣은 아니다(속옷 위에 상의·하의를 겹쳐 입는 것을 전제로 한 베이스).

### 새 원화가 필요하다면 — 규격 (참고용, 지금 만들라는 게 아니라 향후 발주 시 기준)

- 캔버스: `outfit_full`과 동일한 420×512px, 목선(collar) 기준선 `NECK_Y=140`(상의 상단 앵커).
- 상의: 어깨~허리 라인만 그린 투명 PNG, 하의와 자연스럽게 겹치는 여유(허리선 10px 안팎)를
  포함해야 함(현재 `normalize_dress_overlays.py`의 `NECK_RAISE=12`, `SHOE_RAISE=8`과 같은
  여유값 관례).
- 하의: 허리~발목 라인, 상의 하단과 겹치는 여유 포함. 신발과 겹치는 발목 부분도 여유 필요.
- 신발: 발목 아래만, 좌우 한 켤레가 항상 붙어 있는 구도로(현재 신발 오분류 버그 이력—
  좌우가 분리된 크롭은 스크립트가 한쪽을 잘라먹은 전례 있음, 위 "파이프라인 버그 4종" 절
  참고) 그리는 게 안전.
- 캐릭터 kind별(해녀/해남 항해사/해남 기관사/새싹 8체형)로 각각 별도 상의·하의·신발
  세트가 필요 — 체형이 다르면 같은 그림을 재사용할 수 없다(기존 `dress_full`이 새싹
  체형별로 못 넘어간 것과 같은 이유, 위쪽 "캐릭터 의상 시트 3/5/8 정규화" 절 참고).

### 실제로 구현한 것 (에셋이 없는 동안의 완화 조치, 사용자 요청)

가짜 상품을 채우는 대신 요청받은 두 가지만 실제로 바꿨다:

1. **빈 탭 숨김**: `components/store/ClothingTabs.tsx`가 이제 `CLOTHING_TABS` 전체를 항상
   그리지 않고, `ClothingStoreScreen.tsx`가 상품 개수를 세어(`availableTabs`) 1개 이상 있는
   탭만 넘겨준다. 상의/하의/신발은 지금 상품이 0개라 자동으로 숨겨진다 — 하드코딩으로
   "이 세 개를 숨겨라"가 아니라 데이터 기반이라, 나중에 진짜 레이어 에셋이 생겨 상품이
   채워지면 코드 수정 없이 자동으로 다시 나타난다.
2. **전신 의상 재분류**: `lib/domain/clothingStoreCategories.ts`의 `clothingTabFor()`가
   원피스가 아닌 outfit 상품을 기존 `"top"`(상의) 대신 새 `"set"`(세트의상) 탭으로 분류하도록
   변경 — 오버롤/후드티 같은 전신 의상을 "상의"에 넣어두면 "하의는 안 바뀌겠구나"라고
   오해하게 만들기 때문. `CLOTHING_TABS`에 `{ key: "set", label: "세트의상" }` 추가.
   `clothingStoreCategories.test.ts` 기대값도 `"top"` → `"set"`으로 갱신.

`tsc`/`vitest run`(247개)/`next build` 모두 통과.

## 홈 화면 전면 재구현 3차 — GitHub에 새로 업로드된 "홈화면 UI" 24장 에셋 적용

사용자가 GitHub 웹 UI로 `design-assets/홈화면 UI (1~24).png`를 직접 업로드(배포 브랜치에
커밋 `fdf5734`로 반영됨)하고, 이 24장을 실제로 조합해 홈 화면(`/home`)을 다시 만들어달라고
요청했다. 작업 시작 시점에 저장소에 그런 폴더가 전혀 없어서(로컬 클론·GitHub API 양쪽에서
`홈화면 UI`/`home-ui` 문자열 전수 검색 결과 0건, 브랜치도 이 저장소엔 이 2개뿐) 먼저 사용자에게
정확히 어디에 올렸는지 물었고, 사용자가 GitHub 폴더 링크를 보내준 뒤 다시 fetch해서
새 커밋을 확인 — 그 사이에 실제로 업로드가 완료된 것이었다.

### 1) 24장 전수 확인(직접 열어서 확인, 파일명만 보고 판단 안 함)

전부 1254×1254(2장만 예외: 로고 1536×1024, 리본 2172×724) RGBA 투명 PNG. 표로 정리:

| # | 내용 | 크기 | 투명 | 홈 화면 용도 |
|---|---|---:|---|---|
| 1 | 곰돌이 배낭 | 1254×1254 | ✅ | 가방(메뉴/하단바) |
| 2 | 달력+별 | 1254×1254 | ✅ | 이벤트 배너 아이콘 |
| 3 | 햄버거 메뉴(3줄) | 1254×1254 | ✅ | 메뉴(하단바) |
| 4 | 서있는 갈매기 | 1254×1254 | ✅ | 미사용(다른 갈매기 채택) |
| 5 | 돌기둥 | 887×1774 | ✅ | 미사용(테마와 안 어울림) |
| 6 | 배의 타(선박 휠) | 1254×1254 | ✅ | 갑판 광장/갑판(메뉴+하단바) |
| 7 | 2인용 침대 | 1254×1254 | ✅ | 선실(하단바) |
| 8 | 집 아이콘 | 1254×1254 | ✅ | 홈(하단바) |
| 9 | 구명튜브 | 1254×1254 | ✅ | 항해정보 카드 장식(교체) |
| 10 | 트로피 | 1254×1254 | ✅ | 명예의 전당(메뉴) |
| 11 | "출항하기 +$1" 완성 버튼 | 1254×1254 | ✅ | 출항하기 버튼(미출항 상태) |
| 12 | 날아가는 갈매기 | 1254×1254 | ✅ | 헤더 장식 |
| 13 | 요리사 모자 | 1254×1254 | ✅ | 선내식당(메뉴) |
| 14 | 파란 + 버튼 | 1254×1254 | ✅ | 선용금 정보판 + 배지 |
| 15 | 건물(닻 로고) | 1254×1254 | ✅ | (주)해녀해운(메뉴) |
| 16 | 파란 리본(양끝 배의 타 장식, 텍스트 없음) | 2172×724 | ✅ | "나의 항해 정보" 리본 |
| 17 | 완성 로고(은은한 글로우, 등대 없음) | 1536×1024 | ✅(가장자리 alpha=0 실측 확인) | 게임 로고 |
| 18 | 파란 닻 | 1254×1254 | ✅ | D-day 판 "하선" 아이콘 |
| 19 | 등대(재작업본) | 1254×1254 | ✅ | 헤더 장식 |
| 20 | 클립보드 체크리스트 | 1254×1254 | ✅ | 선내업무(메뉴) |
| 21 | 배(깃발+닻) | 1254×1254 | ✅ | 헤더 장식 |
| 22 | 노란 배낭 | 1254×1254 | ✅ | 미사용(1번과 중복) |
| 23 | 뭉게구름 | 1254×1254 | ✅ | 헤더 장식 |
| 24 | 낚싯대+양동이 | 1254×1254 | ✅ | 낚시터(메뉴) |

22장 사용, 2장(5 돌기둥, 22 노란 배낭)은 의도적으로 미사용 — 5번은 파스텔 바다 테마와
안 어울리는 회색 석재 질감이라 억지로 끼워넣지 않았고, 22번은 1번과 같은 용도(가방)라
중복. 한글·공백 경로가 Vercel 배포에서 문제될 수 있어 실제 사용할 22장만
`public/images/home-ui/`에 영문 파일명으로 정리했다(원본 `design-assets/`는 그대로 보존).

### 2) 실제 코드에 적용한 파일과 내용

- `components/home/HomeHeader.tsx`: 로고를 `misc/logo-wordmark.png`(등대가 그려진 옛 버전)
  → `home-ui/logo-glow.png`로 교체(가장자리 alpha=0이라 배경과 자연스럽게 섞임). 로고에
  등대가 없어졌으므로 `home-ui/lighthouse.png`를 헤더 하단에 새로 배치해도 중복이 안 생김
  (직전 라운드엔 옛 로고에 등대가 박혀 있어서 중복 문제로 등대를 뺐었다 — 이번엔 로고 자체가
  바뀌어서 그 제약이 사라졌다). 배는 `home-ui/ship.png`로 교체. 구름 1장·갈매기 1장을
  `home-ui/cloud.png`·`home-ui/seagull-flying.png`로 교체(나머지는 기존 에셋 유지). 선용금
  카드의 "+" 배지를 `home-ui/plus-button.png` 이미지로 교체(기존 GameIcon SVG 대체).
- `components/home/VoyageInfoCard.tsx`: CSS 삼각형+SVG로 만들던 리본을
  `home-ui/ribbon-banner.png`(텍스트 없는 실제 리본 그림) 위에 "나의 항해 정보" 텍스트를
  얹는 방식으로 교체. 구명튜브를 `home-ui/lifebuoy.png`로 교체. "하선" D-day 아이콘을
  GameIcon SVG에서 `home-ui/anchor.png`로 교체. 출항하기 버튼: 미출항 상태는
  `home-ui/departure-button.png`(글자가 이미 그림에 그려져 있음)를 그대로 버튼으로 쓰고,
  출항완료 상태는 그림에 "완료" 문구가 없어서(동적으로 못 바꿈) 기존 점선 원+텍스트를 유지.
- `components/home/EventRow.tsx`: 아이콘을 `icons/book.png`(방명록 용도로 만들어진 아이콘,
  의미상 살짝 억지 매칭이었음)에서 `home-ui/calendar-star.png`(이벤트 용도로 정확히 맞는
  그림)로 교체. 이벤트 없는 빈 상태도 흰 막대가 아니라 같은 카드 톤 + 같은 아이콘으로 표시.
- `components/home/FunctionMenuGrid.tsx`: 9개 장소 중 7개(갑판 광장·선내업무·가방·낚시터·
  선내식당·해녀해운·명예의 전당)를 `home-ui/*.png` 실제 그림으로 교체. 본뿌리·리리양곱창은
  이번 24장에 대응 그림이 없어 기존 GameIcon(`icons/flower.png`, `icons/gopchang.png` —
  이것도 placeholder가 아니라 디자이너가 만든 실제 그림) 유지.
- `components/nav/BottomNav.tsx`: 홈/선실/갑판/가방/메뉴 5개 전부 `home-ui/*.png`로 교체.
- 이 매핑들은 홈 화면 컴포넌트 안에서 `HOME_UI_ICON` 룩업 테이블로만 적용했다 —
  `components/icons/GameIcon.tsx`와 `public/images/icons/*.png`(다른 화면에서도 쓰는 공용
  아이콘)는 전혀 건드리지 않아서, 이번 변경이 홈 화면 밖으로 새지 않는다.

### 3) 실제 발견하고 고친 버그 2건

1. **등대 중복**: 처음 로직대로 새 등대 그림을 헤더에 넣고 스크린샷을 보니 등대가 2개
   보였다 — 원인은 당시 아직 로고를 안 바꾼 상태에서 옛 `logo-wordmark.png`(좌측에 등대
   포함)가 남아있었기 때문. 로고를 `logo-glow.png`로 먼저 교체하니 중복이 사라졌다(위 1번
   참고). 실제 렌더링 스크린샷을 픽셀 단위로 확인하지 않았다면 놓쳤을 문제.
2. **출항하기 버튼 클릭 안 됨(z-index 동률)**: `출항하기` 버튼(`z-10`)과 캐릭터를 감싼
   `/voyage` 링크 안의 캐릭터 행(`z-10`)이 같은 스택 컨텍스트에서 z-index가 동률이라, DOM
   순서상 뒤에 있는 캐릭터 행이 버튼 위에서 클릭을 가로챘다(Playwright로 실제 클릭 시도해서
   타임아웃 발생 → `elementFromPoint`로 좌표 진단 → 원인 확정). 버튼 쪽 z-index를 `z-20`으로
   올려 해결하고, Playwright로 실제 클릭 → "출항 완료" 상태 전환 + "선용금 지급 완료 +$1"
   토스트까지 재확인했다.

### 4) 실제 브라우저 검수

Playwright로 360×740, 390×844, 430×932 세 크기 모두 `/home` 렌더링 확인 — 가로 스크롤 없음,
로고/장식/캐릭터/버튼 잘림·겹침 없음, 하단 네비게이션이 본문을 가리지 않음, 이미지 404
없음(콘솔에 잡힌 404는 전부 사전부터 있던 오디오 파일(`attendance.mp3`, `home.mp3`)과
`/api/ship-events/status` 500 — 홈 UI와 무관, 이번 작업 범위(오디오/DB) 밖). 9개 장소 메뉴 +
5개 하단 네비게이션, 총 12개 고유 라우트 전부 실제로 페이지 로드 200 확인. 출항하기 버튼은
위 버그 수정 후 실제 클릭 → 상태 전환까지 확인.
