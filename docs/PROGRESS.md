...
- 사용자가 새로 보내준 "캐릭터 의상 (1)~(10).png" 10장(가디건/원피스 세트, 가방·신발이 옷
  본체와 분리되어 떠 있는 구도)은 아직 미처리 — 지금의 정규화 스크립트는 "목~신발까지 한
  장에 그려진 의상"을 전제로 하는데, 이 새 세트는 원피스/상의만 그려져 있고 신발이 별도
  플로팅 오브젝트라 같은 파이프라인을 그대로 쓸 수 없음. 신발을 기본 체형의 발 위치에 별도로
  앵커링하는 로직이 추가로 필요 — 다음 세션 작업.
